...
## Build Model
m_shape = (historyPoints, dataX.shape[1]) # 24 hrs, 12 observatory data
...
dfNowcast = pd.DataFrame(columns = ['incHour','loss','real0','pred0','diff0','real1','pred1','diff1','real2','pred2','diff2','real3','pred3','diff3','real4','pred4','diff4','real5','pred5','diff5'])

## Prepare Dataset
dataPeriod = '2020-01-18 ~ 02-07' # 21 days
#dataPeriod = '2020-01-18 ~ 02-14' # 28 days
#dataPeriod = '2020-01-21 ~ 02-21' # 35 days
trainDays  = 21

### Begin of Incremental Training 
numHour = 72 
for incHour in range(0, numHour):
	start, end = 0+incHour, 0+24*trainDays+incHour
	...
	## Train Model
	if os.path.exists('models/nowcasting_lstm.h5'):
		num_epochs = 20 # incremental training
	else:
	...
	## Save Model
#	if not os.path.exists('models/nowcasting_lstm.h5'):
#		model.save_model('models/nowcasting_lstm.h5')
...
#	loss = history.history['loss'][-1]
	...
	if os.path.exists('models/nowcasting.hdf5'):
		model.load_weights("models/nowcasting.hdf5")
	...
	print(ll)
	dfNowcast.loc[len(dfNowcast)] = ll
	dfNowcast.to_csv('out/nowcasting.csv',index=False)
